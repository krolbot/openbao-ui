...
| **7** ◑ | Differentiators | — | Cross-engine **comparison matrix** + **audit-log viewer** (declarative file device → BFF reader → filterable table) done. Deferred: PR-style approval workflows + secret-sync/integrations — both need a stateful control plane (datastore + multi-user) above OpenBao's stateless model |
